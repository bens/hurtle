...
forkProcess :: (Functor m, Monad m)
            => StateT (HurtleState s c l) m (ForkId s c a)
forkProcess = do
    st <- get
            ...
            Block (ForkId cid' fid') k -> do
                ...
                case fid' TS.! forks of
                    ...
                    ProcessFailed err -> do
                        logIt $ PropagatingError cid' cid
